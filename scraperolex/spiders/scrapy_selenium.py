...
class RolexspiderSpider(scrapy.Spider):
    ...
    def parse_models_page(self,response):
        ...
        for watch in watches:
           ...
           #for each watch type, open all drop down menus to prepare page to be scraped
            yield SeleniumRequest(url=watch_model_link,callback=self.parse_watch_page,script="document.querySelector('.css-1tg8aam e1yf0wve3').click()",wait_time=3)
    ...
    def parse_watch_page(self,response):
        ...
        #Pulling price with selenium and beautiful soup separately
        # print('Starting new driver')
        # driver = webdriver.Chrome()
        # driver.get(response.url)
        # page_source = driver.page_source
        # soup = BeautifulSoup(page_source, 'html.parser')
        # p_price = soup.find("p", {"class": "css-2im8jf css-1g545ff e8rn6rx1"}).getText()
        # print('Here')
        # print(p_price)
